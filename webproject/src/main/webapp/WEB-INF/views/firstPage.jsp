    ...
    <div class="king-layout2-main mt15" style="width:960px;">
        <form class="form-horizontal">
            <div class="form-group clearfix ">
                ...
                <div class="col-sm-9">
                    <select name="" id="business" class="form-control bk-valign-top" onchange="changeBizId(this.value)">
                    </select>
                </div>
            </div>
            <div class="form-group clearfix ">
                ...
                <div class="col-sm-9">
                    <input type="text" class="form-control bk-valign-top" id="params" placeholder="请输入脚本参数"></div>
            </div>
            <div class="form-group clearfix ">
                <label class="col-sm-3 control-label bk-lh30 pt0">脚本列表：</label>
                <div class="col-sm-9">
                    <select name="" id="scriptSelect" class="form-control bk-valign-top">
                    </select>
                </div>
            </div>
            <div class="form-group clearfix ">
            ...
            <div class="form-group clearfix">
                <div class="col-sm-9 col-sm-offset-3">
                    <button type="button" class="king-btn mr10  king-success" onclick="runScript()">执行</button>
                </div>
            </div>
        </form>
...
<script>
    ...
    function loadUser() {
        $.ajax({
            method: 'GET',
            url: "${sessionScope.SITE_URL}rest/my/user/info",
    ...
    function loadScript(bizId) {
        $("#scriptSelect").empty();
        $.ajax({
            method: 'GET',
            url: "${sessionScope.SITE_URL}rest/my/script/list?bizId="+bizId,
            success: function(result){
                console.log(result);
                if(result.result==true){
                    $("#scriptSelect").empty();
                    var scriptList = result.data.data;
                    scriptList.forEach(function (item) {
                        var scriptOption = "<option value='"+item.id+"'>"+item.name+"</option> ";
                        $("#scriptSelect").append(scriptOption);
                    });
                }
            },
            dataType: "json",
    ...
    function loadBusiness() {
        $.ajax({
            method: 'GET',
            url: "${sessionScope.SITE_URL}rest/my/business/list",
            success: function(result){
                console.log(result);
                if(result.result==true){
                    $("#business").empty();
                    ...
                    changeBizId(businessList[0].bk_biz_id);
                }
            },
            dataType: "json",
            headers: {'Content-Type': 'application/json'}
    ...
    function loadHost() {
        $.ajax({
            method: 'GET',
            url: "${sessionScope.SITE_URL}rest/my/host/list",
            success: function(result){
                console.log(result);
                if(result.result==true){
                    $("#tableBody").empty();
                    var hostList = result.data.info;
                    var index = 1;
                    hostList.forEach(function (item) {
                        var item = "<tr>\n" +
                            ...
                            "                        </td>\n" +
                            "                        <td>"+index+"</td>\n" +
                            "                        <td>"+item.host.bk_host_innerip+"</td>\n" +
                            "                        <td>"+item.host.bk_os_name+"</td>\n" +
                            "                    </tr>";
                        index ++;
    ...
    function runScript() {
        /*var ipList = [];
        $('#table_demo2 input.checked ').forEach(function () {
            if($(this).attr("checked")==true){
                var tr = $(this).parent().parent();
                var index= tr .children().eq(2).html();
        ...
        $("#table_demo2 input:checkbox").each


        var data = {
            bizId:$("#business").val(),
            scriptId:$("#scriptSelect").val(),
            params:$("#params").val(),
            timeout:$("#timeout").val(),
            account:$("#account").val(),
            ipList:$("#account").val()
